...
class Hit extends Component {
	...
	componentWillReceiveProps(nextProps){
		...
		if(nextProps.hit.codigo !== hit.codigo){
			this.setState({ showCantidad: false, cantidad: 1 });
		}
	}
	//Add Item to cart
	onPressAddToCart(cartItem, e) {
		...
		setTimeout(() => {
			this.props.onAddItemToCart(cartItem, cantidad);
		}, 1000)
		e.preventDefault();
	...
	cancelarItem(e){
		...
		this.setState({ showCantidad: false, loading: false, cantidad: 1 });
	}
				...
				<div className="overlay-wrap overflow-hidden" >
					...
					<Collapse in={showCantidad} timeout={500} > 
						...
						<div style={{display: "block", textAlign: "center", margin: "2rem 0" }}>
							<Button disabled={ hit.cantidad_deposito_item === 0 ? true : false } 
									color="primary" 
									variant="contained" 
									style={{margin: "0 5px"}} 
